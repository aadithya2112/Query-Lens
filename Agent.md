...
- Current product state: `what changed` and `compare` for `cashflow_health_score`, plus `breakdown` for `at_risk_account_count`, are implemented over a built-in sample dataset, with both fixture mode and Docker-backed `database` mode working
- Current engineering priority: pivot the interactive path to an honest LLM-first flow before building `weekly briefing`
...
- Do not present deterministic interactive parsing fallback as the intended end-state
