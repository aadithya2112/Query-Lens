...
- Current product state: `what changed` for `cashflow_health_score` and `breakdown` for `at_risk_account_count` are implemented, with both fixture mode and Docker-backed `database` mode working
- Current engineering priority: Stage 2 breakdown is shipped; `compare` is the next user-visible slice
...
- Do not present dataset onboarding, `compare`, or `weekly briefing` as shipped yet
